...
void show3DObjects(std::vector<BoundingBox>& boundingBoxes,
  ...
  double scale = 0.5;
  cv::Mat topviewImgScaled(imageSize, CV_8UC3, cv::Scalar(255, 255, 255));
  cv::resize(topviewImg, topviewImgScaled, cv::Size(), scale, scale);
  cv::imshow(windowName, topviewImgScaled);
