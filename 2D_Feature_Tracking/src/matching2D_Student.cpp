...
void descKeypoints(vector<cv::KeyPoint>& keypoints,
  ...
  if (descriptorType.compare("BRISK") == 0) {
    ...
    throw runtime_error(
        " Has to be one of BRIEF, BRISK, ORB, AKAZE, FREAK, SIFT, but is" +
        descriptorType);
  }
...
void detKeypointsModern(std::vector<cv::KeyPoint>& keypoints,
  ...
  if (detectorType.compare("FAST") == 0) {
    detector = cv::FastFeatureDetector::create(20);  // threshold = 20
  } else if (detectorType.compare("BRISK") == 0) {
    detector = cv::BRISK::create(30);  // threshold = 30
  } else if (detectorType.compare("ORB") == 0) {
    detector = cv::ORB::create(500);  // num features = 500
    ...
    throw runtime_error(
        " Has to be one of FAST, BRISK, ORB, AKAZE, SIFT, but is" +
        detectorType);
  }
